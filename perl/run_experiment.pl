...
#    $data_file = $outputdir_full . "/data$N.dat";
    ...
    # Cut the first N lines from $data file
#    `head -n $N $data > $data_file`;
...
#    `./c2/bscore -bic -nodes 37 -maxpa 4 -data $data_file -mod_out $model_file -edge_scores .005`;
    ...
    `sed -i '' 's|%DIR|$outputdir_full|' $outputdir_full/gobnilp$N.set`;
...
#    print `grep -A100000  'START MEC' $mec_file > $mec_file2 && diff $mec_file2 ../data/synthetic_examples/alarm_network.MEC`;
}
}
