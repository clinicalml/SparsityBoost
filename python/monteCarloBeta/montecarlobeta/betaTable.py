...
class betaTable(object):
    ...
    def generateDictFromNToGammaList(self):
        ...
        for N in self.NList:
            ...
            probabilityDistPathBasedAtUniform.t_at_spcifiedDivergenceFromMarkedDistInDirectionOfBase(KLDivergence)) 
    ...
    def generateDictFromNTo_KL_divergenceListAndGammaListIncludingGammaGreaterThanEta(self): #tested
        ...
        for N in self.NList:
            ...
            probabilityDistPathBasedAtUniform.t_at_spcifiedDivergenceFromMarkedDistAwayFromBase(KLDivergence)) for KLDivergence in rawKLDivergenceListForGammaGreaterThanEta]
    ...
    def interpolateAndReturnLogBeta(self, N, gamma, currSmallest=0):
		...
		leftNVal = iP.find_ge(self.NList,N)[0]
		if leftNVal is self.NList[-1]:
			rightNVal = leftNVal
		else:
			rightNVal = iP.find_gt(self.NList,leftNVal)[0]
		rightNIndex = self.NFirstIndexHash[rightNVal]
		leftNIndex = self.NFirstIndexHash[leftNVal]
		...
		candidateSmallerLogBeta = np.log( self.betasByAscendingNAscendingGamma['beta'][iP.find_le_withBounds( self.betasByAscendingNAscendingGamma['gamma'],clampedGamma,leftNIndex,rightNIndex )[1]] )
